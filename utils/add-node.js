...
const nodeTypeInPascalCase = nodeTypeSegs.map((val) => val.charAt(0).toUpperCase() + val.substring(1)).join('');
const nodeTypeInCamelCase = nodeTypeSegs
    .map((val, idx) => (idx === 0 ? val : val.charAt(0).toUpperCase() + val.substring(1)))
    .join('');
...
async function generateFiles(fromDir, toDir) {
    ...
    await Promise.all(
        fromDirents.map(async (fromDirent) => {
            ...
            let toFilePath = path.join(toDir, fromDirent.name.replace('node-type', nodeTypeInKebabCase));
            ...
            if (fromDirent.isDirectory()) {
                ...
                if (ext !== '.mustache') {
                    ...
                    toFilePath = path.join(path.dirname(toFilePath), path.basename(toFilePath, ext));
...
async function addNodeToPackageJson() {
    ...
    pkgJsonData['node-red'].nodes[nodeTypeInKebabCase] = `./dist/${nodeTypeInKebabCase}/${nodeTypeInKebabCase}.js`;
